...
// ref claim and report
var version = firebase.database().ref('claim');
var version = firebase.database().ref('report');
...
.service('Services', function($q, $localStorage) {
	...
	this.updateRatingReview = function(dataReview) {
		console.log('dataReview :'+JSON.stringify(dataReview));
		var promise = $q.defer();
		getRefKota('reviewRating').child(dataReview.resto).push({
			'resto': dataReview.resto,
			'indexUser': dataReview.indexUser,
			'rating': dataReview.rating,
			'titleReview': dataReview.titleReview,
			'review' : dataReview.review,
			'username': dataReview.username,
			'userPhotoUrl': dataReview.userPhotoUrl,
			'emoji': dataReview.emoji,
	...
	this.claim = function(data){
		var promise = $q.defer();

		getRefKota('claim/'+data.indexResto+'/'+data.noIdentitas).push({
			'namaPemilik': data.namaPemilik,
			'telepon': data.telepon,
			'jenisIdentitas': data.jenisIdentitas,
			'noIdentitas': data.noIdentitas,
			'indexResto': data.indexResto,
	...
	this.report = function(data){
		...
		getRefKota('report/'+data.indexResto).push({
			'indexResto': data.indexResto,
			'reportList': data.reportList || null,
